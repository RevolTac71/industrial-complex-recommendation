...
if user_input := st.sidebar.chat_input("원하는 산업단지 요건을 자연어로 설명하세요."):
    ...
    try:
        ...
        st.session_state.current_weights["지역여건"] = round(response.get("지역여건", 0.20) * 100.0, 1)
        st.session_state.current_weights["물류여건"] = round(response.get("물류여건", 0.20) * 100.0, 1)
        st.session_state.current_weights["산업혁신여건"] = round(response.get("산업혁신여건", 0.20) * 100.0, 1)
        st.session_state.current_weights["생활정주여건"] = round(response.get("생활정주여건", 0.20) * 100.0, 1)
        st.session_state.current_weights["근로자이동여건"] = round(response.get("근로자이동여건", 0.20) * 100.0, 1)
        ...
        st.session_state.chat_history.append({
            "role": "assistant", 
            "content": f"{recommendation_reason}\n\n추천 분배 결과(총 100점 기준): {st.session_state.current_weights}"
        })
...
if "recommendation_reason" in st.session_state and st.session_state.recommendation_reason:
    ...
    weights_str = " | ".join([f"**{k[:2]}**: {v}%" for k, v in st.session_state.current_weights.items()])
        ...
        for idx, (_, row) in enumerate(top_5_complexes.iterrows()):
            ...
            with st.expander("더 자세한 상세정보 보기"):
                ...
                score_df = pd.DataFrame({
                    ...
                    "배정 점수 (만점)": [
                        st.session_state.current_weights["지역여건"],
                        st.session_state.current_weights["물류여건"],
                        st.session_state.current_weights["산업혁신여건"],
                        st.session_state.current_weights["생활정주여건"],
                        st.session_state.current_weights["근로자이동여건"]
                    ],
                    "반영 점수": [
                        row.get('지역여건_점수', 0.0),
                        row.get('물류여건_점수', 0.0),
