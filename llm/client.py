...
class GeminiLLMClient:
    ...
    def check_multiple_complexes_eligibility(self, complexes_list: list[dict], user_business_description: str) -> dict:
        ...
        system_inst = (
            ...
            "**[의미론적 유사도 심사 지침 - 필독 및 강제 준수]**\n"
            "- 허용 유치업종명은 주로 거시적인 대분류/중분류(예: '기타 기계 및 장비', '금속가공제품', '자동차', '전기장비', '제조업')로 표현됩니다.\n"
            "- 사용자의 구체적인 사업 설명(예: '자동차용 기계 부품 및 금속 가공 제조업') 내 키워드(자동차, 기계, 금속, 가공 등)가 허용 유치업종 목록 내 단어들과 조금이라도 의미론적으로 연관성이 존재한다면(예: '자동차' -> '자동차', '기계' -> '기타 기계 및 장비', '금속/가공' -> '금속가공제품') **절대 '불가'로 판정하지 마십시오. 반드시 '가능' 또는 '조건부 가능'으로 합격 판정해야 합니다.**\n"
            "- 사용자의 사업이 일반적인 제조업(예: 기계, 기공, 장비, 반도체, 정보통신 등)이고 허용 유치업종에도 제조업 대분류가 들어가 있다면, '불가'를 내리지 마십시오. '불가' 판정은 사용자의 사업이 해당 단지의 유치 업종과 아예 하등의 관련이 없는 경우(예: 허용 업종이 '기술서비스' 뿐인데 전통 '주물 공장'을 하려 하거나, 허용 업종이 '가구 제조업' 뿐인데 '정밀 반도체 연구소'를 지으려 하는 등)에만 매우 극단적이고 보수적으로 적용해야 합니다.\n\n"
            "**[판정 기준 규칙]**\n"
            ...
            "**[출력 규칙]**\n"
            "- 반드시 입력으로 주어지는 각각의 'dan_id' 명칭과 'dan_name'을 임의로 축약하거나 생략하지 말고 **그대로 정확히 전달받은 문자열로** JSON 결과의 해당 필드에 기입해 주십시오."
        )
